...
use crate::{block::{Block}, message::{Message, MessageContent, ProposalMessage, VoteMessage}, utils::{BlockHash, sha512_digest}};
    ...
    fn try_finalize_chain(&mut self, block_digest: BlockHash) {
        match self.blocks.entry(block_digest) {
            Entry::Occupied(entry) => {
                ...
                if let Some((parent, _length)) = self.blocks.get(&block.parent_digest) {
                    if let Some((grandparent, _length)) = self.blocks.get(&parent.parent_digest) {
                        if self.can_block_be_finalized(parent, block.creation_epoch - 1)
                            && self.can_block_be_finalized(grandparent, block.creation_epoch - 2)
                        {
        ...
        match self.blocks.entry(*first_longest_chain_head) {
            Entry::Occupied(entry) => {
                ...
                if *curr_longest_length < new_length {
                    ...
                    self.longest_notarized_chain_head_digests.push(new_block.digest());
                }
                info!(
                    target: &self.name(),
...
impl BaseNode for Node {
    fn on_message_received(&mut self, message: &Message, message_queue: &mut VecDeque<Message>) {
        if message.from == self.id {
            info!(target: &self.name(), "Ignoring echoed message - it was sent by this node previously");
            return;
        }
        ...
        match message.content {
            MessageContent::Proposal(ProposalMessage { block }) => {
                if !self.blocks.contains_key(&block.digest()) {
                    ...
                    if self.longest_notarized_chain_head_digests.contains(&block.parent_digest) {
        ...
        if processed_the_unique_message {
            // Echo the message to other nodes; we broadcast after processing to prevent unnecessary growth in messages over time
    ...
    fn at_time(&mut self, epoch: Epoch, message_queue: &mut VecDeque<Message>) {
        ...
        if self.is_leader(epoch) {
            ...
            let first_longest_chain_head_digest = self.longest_notarized_chain_head_digests.first().unwrap();
